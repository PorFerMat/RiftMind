...
import { checkLiveGameStatus, getSmartRecommendations, IS_USING_GRID_API } from './services/gridService'; // Import API flag
import { fetchAnalysis, AnalysisResponse, checkBackendHealth } from './services/api'; // Import health check
...
const App: React.FC = () => {
  ...
  // --- INITIAL CHECK ---
  useEffect(() => {
    checkBackendHealth().then(isOnline => setBackendStatus(isOnline ? 'online' : 'offline'));
  }, []);
      ...
      <header className="h-16 border-b border-gray-800 bg-hex-dark/80 backdrop-blur flex items-center justify-between px-6 sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col cursor-pointer" onClick={() => setView('draft')}>
            ...
            {/* STATUS INDICATORS */}
            <div className="flex gap-3 text-[10px] font-bold tracking-widest uppercase mt-0.5">
              <span className={backendStatus === 'online' ? "text-green-400" : "text-red-500 animate-pulse"}>
                PYTHON: {backendStatus === 'online' ? 'CONNECTED' : 'OFFLINE'}
              </span>
              <span className="text-gray-600">|</span>
              <span className={IS_USING_GRID_API ? "text-green-400" : "text-yellow-500"}>
                GRID: {IS_USING_GRID_API ? 'LIVE' : 'SIMULATED'}
              </span>
            </div>
          </div>
        </div>
